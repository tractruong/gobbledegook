...
Server::Server(const std::string &serviceName, const std::string &advertisingName, const std::string &advertisingShortName, 
	GGKServerDataGetter getter, GGKServerDataSetter setter, const char *filename)
{
	// Save our names
	...
	// [tt] read service info from json file
	std::ifstream file(filename);
	nlohmann::json services;
	file >> services;
	auto &j1 = services["services"];

    for (unsigned int i = 0; i < j1.size(); i++)
    {
        auto &s = j1[i];
        Logger::debug(SSTR <<".gattServiceBegin");
        Logger::debug(SSTR <<"  " << s["name"]); 
        Logger::debug(SSTR <<"  " << s["uuid"]);

	auto ob_service = objects.back().gattServiceBegin(std::string(s["name"]),std::string(s["uuid"]));

        auto &s1 = s["characteristics"];
        for (unsigned int k = 0; k < s1.size(); k++)
        {
        auto &c = s1[k];
        Logger::debug(SSTR <<  "  .gattCharacteristicBegin"); 
        Logger::debug(SSTR << "    " << c["name"]);
        Logger::debug(SSTR << "    " << c["uuid"]);
        Logger::debug(SSTR << "    " << c["flags"]);
        Logger::debug(SSTR << "    " << c["default-value"]);
	std::vector<std::string> flags;
	for (unsigned int l = 0 ; l < c["flags"].size(); l ++)
	{
		flags.push_back(c["flags"][l]);
	}

	auto &ob_char = ob_service.gattCharacteristicBegin(c["name"].get<std::string>(), GattUuid(c["uuid"].get<std::string>()), flags);
	ob_char.pPath = std::string(s["name"]) + std::string("/") + std::string(c["name"]);

	dataSetter(ob_char.pPath.c_str(), 
				c["default-value"].get<std::string>().c_str());

	for (unsigned int l = 0 ; l < flags.size(); l++)
	{
		if (flags[l] == "read")
		{
			ob_char.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				const char* pData = self.getDataPointer<const char*>(self.pPath.c_str(), "");
				self.methodReturnValue(pInvocation, pData, true);
			});
		}
		if (flags[l] == "write")
		{
			ob_char.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				GVariant *pAyBuffer = g_variant_get_child_value(pParameters, 0);
				self.setDataPointer(self.pPath.c_str(), Utils::stringFromGVariantByteArray(pAyBuffer).c_str());
		...
		if (flags[l] == "notify")
		{
			ob_char.onUpdatedValue(CHARACTERISTIC_UPDATED_VALUE_CALLBACK_LAMBDA
			{
				const char *pData = self.getDataPointer<const char *>(self.pPath.c_str(), "");
				self.sendChangeNotificationValue(pConnection, pData);
				return true;
			});
		}
        ...
        Logger::debug("  .gattCharacteristicEnd");
        }
        Logger::debug(".gattServiceEnd()");
    }
